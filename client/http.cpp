...
bool HttpProto::httpRequest(void* data, unsigned long datasize, char* response, int *len)
{
...
bool HttpProto::postTest(string filename,char * pfilebuffer,int filesize)
{
    ...
    ret = httpRequest(postentity, entitysize, response,&backsize);
...
bool HttpProto::postFile(string filename, char* backdata, int* backsize) {
...
bool HttpProto::getCmd(const char* cmd, char* backdata, int* backsize) {
    int ret = 0;
...
//     mbstowcs(url, base64.c_str(), sizeof(url) / sizeof(WCHAR));
    ...
    ret = httpRequest(0, 0, backdata, backsize);
    return ret;
}



bool HttpProto::getCmdStr(const char* cmd,const char * str, char* backdata, int* backsize) {
	int ret = 0;
...
bool HttpProto::getSubCmd(const char* cmd, const char* subcmd, char* backdata, int* backsize) {
	int ret = 0;
...
bool HttpProto::postCmd(const char * cmd,char * backdata, int* backsize) {
...
bool HttpProto::postSubCmd(const char* cmd, const char * subcmd,char* backdata, int* backsize) {
	...
	ret = pack.cmdWrapper(0, 0, cmd,subcmd, &data, &datasize);
...
bool HttpProto::postCmdStr(const char* cmd, const char* subcmd, char* backdata, int* backsize) {
	...
	ret = pack.cmdStrWrapper(0, 0, cmd, subcmd, &data, &datasize);
